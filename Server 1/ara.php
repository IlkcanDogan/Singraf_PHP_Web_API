...
if($_metot == "GET"){
	...
	$start = urldecode(Temizle(trim($_GET["start"])));
	...
	if($text != null && $start != null){
		...
		$_kod = 200;
		$sql = "CALL sp_MUSIC_SEARCH('$text','$start')";
		...
		if(mysqli_num_rows($output) > 0){
			while($satir = mysqli_fetch_assoc($output)){
				...
				$bilgilerArray = array(
					...
					"VIDEO" => "http://165.22.77.175/server2/videos/".$VIDEO_NO.".mp4",
					"THUMB" => "http://165.22.77.175/server2/thumb/".$VIDEO_NO.".jpg",
